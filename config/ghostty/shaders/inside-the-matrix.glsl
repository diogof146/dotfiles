                    ...
                    if (v >= 0.0 && v < 1.0) {
                        ...
                        if (char_hash.x >= 0.1 || c == 0.) {  //10% of missed symbols
                            ...
                            if (a > 0.) {
                                ...
                                vec3 baseColor = vec3(0.0, 0.384, 0.443);
                                vec3 col = (c == 0. ? baseColor * 1.5 : baseColor) / attenuation;
                                float a1 = result.a;
                                result.a = a1 + (1. - a1) * a;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
  ...
  float alpha = step(length(terminalColor.rgb), 0.1);  // Use 0.1 like your other shaders
